...
export class muEtl {
  ...
  async init() {
    try {
      ...
      this.logger.info(`Database connection established for ETL type: ${this.type}`);
    }
    catch (error) {
  ...
  public etlProcess(etlInput : EtlInput): Promise<EtlOutput>{
    ...
    return new Promise <EtlOutput>(async (resolve, reject)=>{
      try {
        ...
        // Get the CSV stream
        const csvStream = getCsvStream(etlInput.dataLocation);        
      ...
      resolve({
        issueIds: parsedData.map(data => 
          data.issues.map(issue => issue.issueId)
        ).flat(),
        ...
        this.db.close();
        this.logger.info(`Database connection closed for ETL type: ${this.type}`);
      }
